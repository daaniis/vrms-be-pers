...
import { CreateNonTranslationDto } from './dto/create-freelance.dto';
import { UpdateNonTranslationDto } from './dto/update-freelance.dto';
...
export class NonTranslationService {
  constructor(
  ...
  async createNonTranslation(createNonTranslationDto: CreateNonTranslationDto) {
    ...
    const missingFields = mandatoryFields.filter(
      (field) => !createNonTranslationDto[field],
    );
    ...
    const existingEmail = await this.prismaService.freelance.findFirst({
      where: {
        email: createNonTranslationDto.email,
    ...
    const country_id = Number(createNonTranslationDto.country_id);
    const state_id = Number(createNonTranslationDto.state_id);
    const city_id = Number(createNonTranslationDto.city_id);
    const currency_id = createNonTranslationDto.currency_id
      ? Number(createNonTranslationDto.currency_id)
      : null;
    const tools = createNonTranslationDto.tools;
    let list_rate = createNonTranslationDto.list_rate;
    ...
    try {
      const createdFreelance = await this.prismaService.freelance.create({
        data: {
          ...
          full_name: createNonTranslationDto.full_name,
          whatsapp: createNonTranslationDto.whatsapp,
          nickname: createNonTranslationDto.nickname,
          email: createNonTranslationDto.email,
          specialization_on: createNonTranslationDto.specialization_on,
          tools: tools,
          district: createNonTranslationDto.district,
          postal_code: createNonTranslationDto.postal_code,
          full_address: convertEmptyStringToNull(
            createNonTranslationDto.full_address,
          ),
          bank_name: convertEmptyStringToNull(
            createNonTranslationDto.bank_name,
          ),
          branch_office: convertEmptyStringToNull(
            createNonTranslationDto.branch_office,
          ),
          account_holder_name: convertEmptyStringToNull(
            createNonTranslationDto.account_holder_name,
          ),
          account_number: convertEmptyStringToNull(
            createNonTranslationDto.account_number,
          ),
          name_tax: convertEmptyStringToNull(createNonTranslationDto.name_tax),
          resource_status: convertEmptyStringToNull(
            createNonTranslationDto.resource_status,
          ),
          npwp_number: convertEmptyStringToNull(
            createNonTranslationDto.npwp_number,
          ),
          country_id: country_id,
          state_id: state_id,
          city_id: city_id,
    ...
    for (const field of fields) {
      ...
      if (newData[field] !== oldData?.[field]) {
        await this.logService.create({
          menu_name: 'Resource Manager - Non Translation',
  ...
  async importNonTranslation(file: Express.Multer.File) {
    if (!file) {
  ...
  async exportNonTranslation(format: string, res: Response) {
    ...
    let fileName = `freelance-non-translation-${new Date().toISOString()}`;
    let data;
  ...
  async findAllNonTranslation(getNonTranslationDto: GetAllDto) {
    const { limit, page, search, sort_order } = getNonTranslationDto;
  ...
  async findOneNonTranslation(
    ...
    if (
      !freelance ||
      freelance.type_freelance !== TypeFreelance.NonTranslation
    ) {
      return null;
    }
  ...
  async updateNonTranslation(
    ...
    // Validasi state_id
    if ('state_id' in data) {
      const countryId = data.country_id ?? freelance.country_id;
    ...
    if ('tools' in data && Array.isArray(data.tools)) {
      const toolsExist = await this.prismaService.tool.findMany({
        where: {
          tool_name: { in: data.tools as string[] },
        },
      });
      if (toolsExist.length !== data.tools.length) {
    ...
    const updatedNonTranslation = await this.prismaService.freelance.update({
      where: { freelance_id },
      data: updateData,
    });
    // RECORD LOG
    ...
    await this.logFieldUpdate(
      updatedNonTranslation,
      freelance,
      userEmail,
      'Update',
    );
    return updatedNonTranslation;
  }
  ...
  async uploadAttachmentNonTranslation(
    ...
    if (!freelance) {
      throw new NotFoundException(
        'Freelance not found or is not a Non Translation.',
      );
    }
  ...
  async downloadAttachmentNameNonTranslation(
    freelance_id: string,
  ...
  async getRatingsNonTranslation(
    freelance_id: string,
    getSubmitRatingNonTranslationDto: GetAllDto,
  ) {
    const freelance = await this.prismaService.freelance.findUnique({
      where: {
        freelance_id,
    ...
    const { limit, page, search, sort_order } =
  ...
  async getPMNotesNonTranslation(
    freelance_id: string,
    getPMNotesNonTranslationDto: GetAllDto,
  ) {
    const freelance = await this.prismaService.freelance.findUnique({
      where: { freelance_id, type_freelance: TypeFreelance.NonTranslation },
    });
    ...
    const { limit, page, search, sort_order } = getPMNotesNonTranslationDto;
  ...
  async getOnePMNoteNonTranslation(freelance_id: string, pm_notes_id: string) {
    const freelance = await this.prismaService.freelance.findUnique({
      where: { freelance_id, type_freelance: TypeFreelance.NonTranslation },
    });
